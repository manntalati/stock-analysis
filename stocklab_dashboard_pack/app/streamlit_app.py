...
ADV_PATH = os.path.join(DATA_DIR, "raw", "adv.parquet")
...
def sidebar_controls(scores):
    ...
    show_pred = st.sidebar.checkbox("Show prediction column", value=True)
    return d, top_n, show_pred

def page_overview(scores, preds, equity, summary):
    d, top_n, show_pred = sidebar_controls(scores)
    row = scores.loc[d]
    if isinstance(row, pd.DataFrame):
        row = row.iloc[-1]
    df = row.sort_values(ascending=False).head(top_n).to_frame("score")
    if show_pred and preds is not None and d in preds.index:
        pr = preds.loc[d]
        if isinstance(pr, pd.DataFrame):
            pr = pr.iloc[-1]
        df["pred_5d"] = pr.reindex(df.index)
    st.subheader(f"Top {top_n} Picks for {d.date()}")
    st.dataframe(df.style.format({"score":"{:.3f}","pred_5d":"{:.4f}"}), use_container_width=True)
...
def page_watchlist(scores, preds, prices):
    ...
    if len(new_list)>0:
        ...
        if preds is not None and d in preds.index:
            ...
            if isinstance(pr, pd.DataFrame):
                pr = pr.iloc[-1]
            sub["pred_5d"] = pr.reindex(sub.index)
        st.markdown(f"Signals for {d.date()}")
        st.dataframe(sub.style.format({"score":"{:.3f}","pred_5d":"{:.4f}"}), use_container_width=True)
...
def main():
    ...
    prices = load_prices()
    with tab1:
        page_overview(scores, preds, equity, summary)
    with tab2:
